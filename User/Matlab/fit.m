...
R_w = 0.0752;                                % 驱动轮半径                  （单位：m）
R_l = 0.422/2;                               % 两个驱动轮之间距离/2         （单位：m）
l_c = 0.126;                                % 机体质心到腿部关节中心点距离  （单位：m）
m_w = 0.21; m_l = 1.62; m_b = 13.85;      % 驱动轮质量 腿部质量 机体质量  （单位：kg）
I_w = 0.00038332661;                           % 驱动轮转动惯量               （单位：kg m^2）
I_b = 0.462592306;             % m_b*(0.40^2+0.13^2)/12.0;     机体转动惯量(自然坐标系法向)  （单位：kg m^2）
I_z = 0.314373184;             % m_b*(0.40^2+0.325^2)/12.0;    机器人z轴转动惯量
...
% 短腿长参数
% Q = diag([300 100 300 100 10000 600 10000 600 100000 2000]);
% 长腿长参数
Q = diag([300 100 300 100 10000 1200 10000 1200 100000 2000]);

% Q = diag([60.00, 100.00, 80.00, 20.00, 2000.00, 70.00, 2000.00, 70.00, 10000.00, 80.00]);
...
%左右不同腿长的所有组合，计算出对应的K
L_vals = 0.10:0.01:0.37;
R_vals = 0.10:0.01:0.37;
